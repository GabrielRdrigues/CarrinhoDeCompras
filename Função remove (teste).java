...
public class CarrinhoCompra{
    ...
    public void remove_item(String n, int qtd){
        for (int i = 0; i < itens_do_carrinho.size(); i++) {
            ProdutoEstoque item = itens_do_carrinho.get(i);
            if (item.nome.equals(n)) {
                if (item.quantidade > qtd) {
                    item.quantidade -= qtd; // Reduz a quantidade do item no carrinho
                } else {
                    itens_do_carrinho.remove(i); // Remove o item completamente do carrinho
                }
                break;
            }
        }
